...
import { puranas } from '@/lib/data/puranas'
import { ExternalLink, ChevronDown } from 'lucide-react'
import Link from 'next/link'

export default function PuranasSection() {
  ...
  return (
    <section className="relative min-h-screen py-20 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <Image
          src="/images/backgrounds/vishnu.jpg"
          alt="Vishnu"
          fill
      ...
      <div className="relative z-20 container mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-text-light/80 max-w-3xl mx-auto">
            Ancient texts containing myths, legends, and stories of gods, goddesses, and heroes
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {puranas.map((purana, index) => (
            <motion.div
              key={purana.id}
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button
                    onClick={() => setExpandedId(expandedId === purana.id ? null : purana.id)}
                    className="text-primary-gold hover:text-primary-saffron transition-colors"
                  >
                    <ChevronDown
                      size={20}
                      className={`transition-transform ${expandedId === purana.id ? 'rotate-180' : ''}`}
                    />
                  </button>
                </div>
                ...
                <AnimatePresence>
                  {expandedId === purana.id && (
                    ...
                    >
                      ...
                      >
                        <span>Source: {purana.source}</span>
