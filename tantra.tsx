...
import { tantras } from '@/lib/data/tantra'
import { ExternalLink, ChevronDown } from 'lucide-react'
import Link from 'next/link'

export default function TantraSection() {
  const [expandedId, setExpandedId] = useState<string | null>(null)

  return (
    <section className="relative min-h-screen py-20 overflow-hidden">
      {/* Background Image with Overlay - Tridevi */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-overlay z-10" />
        <Image
          src="/images/backgrounds/tridevi.jpg"
          alt="Tridevi - Lakshmi, Saraswati, Parvati"
          fill
      ...
      <div className="relative z-20 container mx-auto px-4">
        ...
        >
          ...
          <p className="text-xl text-text-light/80 max-w-3xl mx-auto">
            Sacred texts containing esoteric knowledge, rituals, and spiritual practices
          </p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {tantras.map((tantra, index) => (
            <motion.div
              key={tantra.id}
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="p-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button
                    onClick={() => setExpandedId(expandedId === tantra.id ? null : tantra.id)}
                    className="text-primary-gold hover:text-primary-saffron transition-colors"
                  >
                    <ChevronDown
                      size={20}
                      className={`transition-transform ${expandedId === tantra.id ? 'rotate-180' : ''}`}
                    />
                  </button>
                </div>
                ...
                <AnimatePresence>
                  {expandedId === tantra.id && (
                    ...
                    >
                      ...
                      >
                        <span>Source: {tantra.source}</span>
